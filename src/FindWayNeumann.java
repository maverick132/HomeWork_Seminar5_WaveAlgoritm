...
public class FindWayNeumann {

    final static int[][] cellAroundNeumann = { // Реализация окрестности фон Неймана
            {-1, 0}, {0, -1}, {0, 1}, {1, 0},};
    static int step = 0;
    ...
    public static void findWay(Field field) throws IOException {
        ...
        while (!isFinalCellFind) {
            ...
            if (field.isValidCoordinates(x, y)) {
                ...
                fillCellAroundNeumann(field, cell);
                field.printFieldToFile(2);
            }
    ...
    public static void fillCellAroundNeumann(Field field, Cell cell) {
        for (int i = 0; i < 4; i++) {
